...
mod tests {
    ...
    fn test_parse_all_priorities() {
        let queries = vec![
            "CREATE ISSUE OF KIND bug IN test WITH TITLE 'Test' PRIORITY critical",
            "CREATE ISSUE OF KIND bug IN test WITH TITLE 'Test' PRIORITY high",
            "CREATE ISSUE OF KIND bug IN test WITH TITLE 'Test' PRIORITY medium",
            "CREATE ISSUE OF KIND bug IN test WITH TITLE 'Test' PRIORITY low",
        ];
    ...
    fn test_integration_workflow() {
        let queries = vec![
            ...
            "CREATE ISSUE OF KIND bug IN backend WITH TITLE 'Bug fix' PRIORITY high ASSIGNEE alice",
    ...
    fn test_string_with_multiple_escapes() {
        let query =
            r"CREATE ISSUE OF KIND bug IN test WITH TITLE 'Line1\nLine2\tTab\rReturn\\Backslash'";
        let result = parse_query(query).unwrap();
        insta::assert_debug_snapshot!(&result);
    }
    ...
    fn test_all_field_keywords_in_create() {
        let query = "CREATE ISSUE OF KIND bug IN test WITH TITLE 'T' DESCRIPTION 'D' PRIORITY high ASSIGNEE alice";
        let result = parse_query(query).unwrap();
        insta::assert_debug_snapshot!(&result);
    }
    ...
    fn test_special_characters_in_strings() {
        let query = r"CREATE ISSUE OF KIND bug IN test WITH TITLE 'Special chars: !@#$%^&*()_+-={}[]|:;<>?,./~`'";
        let result = parse_query(query).unwrap();
        insta::assert_debug_snapshot!(&result);
    }
    ...
    fn test_priority_in_different_cases() {
        let queries = vec![
            "CREATE ISSUE OF KIND bug IN test WITH TITLE 'T' PRIORITY critical",
            "CREATE ISSUE OF KIND bug IN test WITH TITLE 'T' PRIORITY CRITICAL",
            "CREATE ISSUE OF KIND bug IN test WITH TITLE 'T' PRIORITY Critical",
        ];
        for query in queries {
